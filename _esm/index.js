...
export default function createAuthModule(_ref) {
  ...
  var parseHash = Observable.create(function (obs) {
    return auth0.parseHash(function (err, payload) {
      ...
      errCheck.swap(always(payloadCheck), Failures.Callback).map(calcExipryTime).either(callError(obs), callNextComplete(obs));
    });
  });
  ...
  var checkSession = function checkSession() {
    ...
    return Observable.create(function (obs) {
      return auth0.checkSession(options, function (err, result) {
        return fromNullable(err).swap(always(result), always(Failures.SSO)).map(calcExipryTime).either(callError(obs), callNextComplete(obs));
      });
    });
  };
